...
// Uploads a student's ID photo to Firebase Storage and returns a public
// download URL to save on the request document. Storage is enabled
// automatically on the free Spark plan — no extra setup beyond Firestore.
export async function uploadIdPhoto(file, refNumber) {
  const path = `id-photos/${refNumber}-${Date.now()}-${file.name}`;
...
export function subscribeCollection(key, callback, onError) {
  return onSnapshot(
    collection(db, "docutrack_" + key),
    (snap) => {
      const items = [];
      snap.forEach((d) => items.push(d.data()));
      callback(items);
    ...
    (err) => {
      console.error(`docutrack_${key} subscription error:`, err);
...
/*
STORAGE RULES — paste this into Firebase Console > Storage > Rules
(if Storage isn't enabled yet, go to Build > Storage > Get started first,
same free Spark plan, no extra setup):

rules_version = '2';
service firebase.storage {
  match /b/{bucket}/o {
    match /id-photos/{fileName} {
      allow read, write: if true;
    }
  }
}

Same pilot-only caveat as the Firestore rules above — tighten before any
real rollout.
*/
