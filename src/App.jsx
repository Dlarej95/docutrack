...
import { loadCollection, saveItem, deleteItem, loadSingleton, saveSingleton, subscribeCollection } from "./firebase.js";
...
// ── LANDING ───────────────────────────────────────────────────────────────────
function LandingView({onGo,pendingCount}){
  return(
    <div style={{height:"100%",overflowY:"auto",background:`linear-gradient(155deg,${C.navyD},${C.navyL})`,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"2rem 1.5rem"}}>
      ...
      <p style={{fontSize:13,color:"rgba(255,255,255,.65)",margin:pendingCount>0?"0 0 8px":"0 0 1.75rem"}}>Request and track your documents in real time</p>
      {pendingCount>0&&(
        <div style={{fontSize:12,color:C.gold,background:"rgba(197,155,44,.12)",border:"1px solid rgba(197,155,44,.3)",borderRadius:99,padding:"4px 12px",marginBottom:"1.25rem",display:"flex",alignItems:"center",gap:6}}>
          <i className="ti ti-clock" style={{fontSize:13}}/>Currently processing: {pendingCount} request{pendingCount===1?"":"s"}
        </div>
      )}
...
// ── REQUEST FORM ──────────────────────────────────────────────────────────────
function RequestView({onSubmit,onBack}){
  ...
  const submit=()=>{
    if(!name.trim()||!studentId.trim()||!contact.trim())return;
    const ref=genRef();
    const req={
      ref,name:name.trim(),studentId:studentId.trim(),docType,docName:doc.name,fee:doc.fee*copies,
      copies,purpose:purpose.trim()||"Not specified",contact:contact.trim(),
  ...
  return(
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:C.g50}}>
      ...
      <div style={{flex:1,overflowY:"auto",padding:16}}>
        ...
        <div style={{marginBottom:16}}><label style={lbl}>Contact Number or Email *</label><input value={contact} onChange={e=>setContact(e.target.value)} placeholder="09171234567 or email@example.com" style={inp}/></div>
        ...
        <button onClick={submit} disabled={!name.trim()||!studentId.trim()||!contact.trim()} style={{width:"100%",padding:13,borderRadius:9,background:name.trim()&&studentId.trim()&&contact.trim()?C.navy:C.g300,border:"none",color:C.white,fontSize:14,fontWeight:700,cursor:name.trim()&&studentId.trim()&&contact.trim()?"pointer":"not-allowed"}}>
          Submit Request
        </button>
      </div>
    </div>
...
function TrackView({requests,notifications,onSubmitSurvey,onBack}){
  ...
  const estimatedReady=(()=>{
    if(!found||found.status==="released")return null;
    const doc=DOC_TYPES.find(d=>d.id===found.docType);
    if(!doc)return null;
    const d=new Date(found.createdAt);
    d.setTime(d.getTime()+doc.estDays*86400000);
    return d;
  })();
        ...
        {found&&(
          <div>
            <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.g200}`,padding:16,marginBottom:16}}>
              ...
              {estimatedReady&&(
                <div style={{fontSize:12,color:C.navy,marginTop:6,paddingTop:8,borderTop:`1px solid ${C.g100}`,display:"flex",alignItems:"center",gap:6}}>
                  <i className="ti ti-calendar-time" style={{fontSize:14}}/>
                  Expected ready by <b>{estimatedReady.toLocaleDateString(undefined,{month:"short",day:"numeric",year:"numeric"})}</b>
                </div>
              )}
            </div>
...
// ── ADMIN DASHBOARD ───────────────────────────────────────────────────────────
function AdminView({currentUser,requests,staff,notifications,emailConfig,owner,auditLog,surveys,onUpdateStatus,onDeleteRequest,onClearQueue,onSaveStaff,onSaveEmailConfig,onSaveOwner,onLogout}){
  ...
  return(
    <div style={{height:"100%",display:"flex",flexDirection:"column",background:C.g50,position:"relative"}}>
      ...
      <div style={{display:"flex",background:C.white,borderBottom:`1px solid ${C.g200}`,flexShrink:0}}>
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)} style={{padding:"10px 14px",border:"none",background:"none",cursor:"pointer",fontSize:12,fontWeight:tab===t.id?600:400,color:tab===t.id?C.navy:C.g500,borderBottom:tab===t.id?`2px solid ${C.navy}`:"2px solid transparent",display:"flex",alignItems:"center",gap:5}}>
          ...
          {filtered.length===0?(
            ...
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {filtered.slice().reverse().map(r=>{
                ...
                return(
                  <div key={r.ref} style={{background:C.white,borderRadius:10,border:`1px solid ${C.g200}`,padding:"12px 14px",display:"flex",gap:8,alignItems:"flex-start"}}>
        ...
        return(
          <div style={{flex:1,overflowY:"auto",padding:14}}>
            ...
            <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.g200}`,padding:16,marginBottom:12}}>
              ...
              <div style={{fontSize:12,color:C.g500}}>Contact: {selected.contact}</div>
            </div>
            <div style={{background:C.white,borderRadius:12,border:`1px solid ${C.g200}`,padding:16,marginBottom:12}}>
      ...
      {tab==="auditlog"&&isSuperAdmin&&<AuditLogTab entries={auditLog}/>}
    </div>
  );
}
...
export default function DocuTrackApp(){
  ...
  useEffect(()=>{
    ...
    const unsubAudit=subscribeCollection("auditlog",setAuditLog);
    const unsubSurveys=subscribeCollection("surveys",setSurveys);
    (async()=>{
  ...
  return(
    <div style={{height:"100vh",overflow:"hidden",fontFamily:"'Inter',system-ui,-apple-system,sans-serif"}}>
      {view==="landing"&&<LandingView onGo={setView} pendingCount={requests.filter(r=>r.status!=="released").length}/>}
      {view==="request"&&<RequestView onSubmit={addRequest} onBack={()=>setView("landing")}/>}
      ...
      {view==="admin"&&currentUser&&<AdminView currentUser={currentUser} requests={requests} staff={staff} notifications={notifications} emailConfig={emailConfig} owner={owner} auditLog={auditLog} surveys={surveys} onUpdateStatus={updateStatus} onDeleteRequest={deleteRequest} onClearQueue={clearQueue} onSaveStaff={saveStaff} onSaveEmailConfig={saveEmailConfig} onSaveOwner={saveOwner} onLogout={logout}/>}
